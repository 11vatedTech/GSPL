...
'  for (var _i = 0, _vals = Array.from(nodes.values()), _n; _i < _vals.length; _i++) {',
'    _n = _vals[_i]; if (_n.attributes && _n.attributes.section === section) return _n;',
'  }',
'  return undefined;',
'}',
'',
...
'  });',
'',
'  function readMetaSection(section, defaultVal) {',
'    var node = findMetaNode(graph.nodes, section);',
'    return (node && node.value) || defaultVal;',
'  }',
'',
'  var constraints = readMetaSection("constraints", { valueRanges: [], structuralConditions: [], targetRestrictions: [], performanceBudgets: [], compatibilityConditions: [] });',
'  var dependencies = readMetaSection("dependencies", { contextRefs: [], knowledgeRefs: [], ruleSetRefs: [], targetContracts: [] });',
'  var entropy = readMetaSection("entropy", { algorithm: "gspl-splitmix64", algorithmVersion: "1.0", rootSeed: "", channels: [] });',
'  var lineage = readMetaSection("lineage", { operation: "primordial", parents: [], generation: 0 });',
'  var provenance = readMetaSection("provenance", { canonVersion: ctx.canonVersion });',
'  var resourceBudget = readMetaSection("resourceBudget", {});',
'  var effectPermissions = readMetaSection("effectPermissions", { filesystem: "none", processExecution: false, networkAccess: false, environmentAccess: false, timeAccess: false, foreignCodeExecution: false, nativeExtensions: false, modelInference: false });',
'  var validationRequirements = readMetaSection("validationRequirements", undefined);',
'  var compatibilityRequirements = readMetaSection("compatibilityRequirements", undefined);',
'',
...
'}',
'',
'export function verifyIndependentReconstruction(originalSeed, originalBytes, graph, ctx) {',
...
'  var fieldErrors = [];',
'  if (!match) {',
'    fieldErrors.push("Bytes: orig=" + ob.length + " recon=" + rb.length);',
'    if (JSON.stringify(originalSeed.payload) !== JSON.stringify(result.seed.payload)) fieldErrors.push("payload");',
'    if (JSON.stringify(originalSeed.constraints) !== JSON.stringify(result.seed.constraints)) fieldErrors.push("constraints");',
'    if (JSON.stringify(originalSeed.dependencies) !== JSON.stringify(result.seed.dependencies)) fieldErrors.push("dependencies");',
'  }',
'  return { ok: match && result.ok, originalBytes: ob, reconstructedBytes: rb, bytesMatch: match, errors: result.errors.concat(fieldErrors), fieldErrors: fieldErrors };',
'}',
];

fs.writeFileSync('scripts/reconstructor-body.txt', body.join('\n'));
console.log('Wrote reconstructor body (' + body.length + ' lines)');
