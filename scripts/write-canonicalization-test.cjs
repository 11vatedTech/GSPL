...
fs.writeFileSync(d + '/canonicalization.test.ts', `/** Comprehensive canonicalization tests — Prompt 2 §3, §10 */
...
import { verifySeedRoundTrip } from "../src/ir-reconstructor.js";
...
describe("Seed to IR to Seed Round Trip", function() {
  ...
  for (var i = 0; i < fixtures.length; i++) {
    ...
    it(f.name + ": roundtrip preserves canonical bytes", function() {
      ...
      var rt = verifySeedRoundTrip(normalized, ir);
...
console.log('Wrote canonicalization.test.ts (' + fs.statSync(d + '/canonicalization.test.ts').size + ' bytes)');
