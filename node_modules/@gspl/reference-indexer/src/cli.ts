...
function parseArgs(argv: readonly string[]): CliOptions | null {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === '--output' && i + 1 < argv.length) outputPath = argv[++i];
  ...
  if (!configPath || !outputPath) return null;
  return { configPath, outputPath, repoRoot };
}

function help(): void {
  process.stdout.write(
    `gspl-index — deterministic reference repository indexer\n\n` +
...
export async function main(argv: readonly string[]): Promise<number> {
  ...
  const raw = JSON.parse(configText) as { repos: Array<{ id: string; name: string; sourceType?: string; source: string; root?: string; description?: string }> };

  // Keep original (repo-relative) paths for the manifest. Resolve for scanning only.
  const manifestRepos: RepoConfig[] = raw.repos.map((r) => ({
    id: r.id,
    name: r.name,
    sourceType: (r.sourceType as 'directory' | 'zip') ?? 'directory',
    source: r.source,
    description: r.description,
  }));

  const filesByRepo = new Map<string, ReferenceFileRecord[]>();
  for (const r of raw.repos) {
    ...
    filesByRepo.set(repo.id, result.files as any[]);
  }
  ...
  process.stderr.write(
    `[gspl-index] wrote ${opts.outputPath}\n` +
