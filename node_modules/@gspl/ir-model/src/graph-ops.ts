...
export function createIrGraph(metadata: IrGraphMetadata): GsplIrGraph {
  const rootId = 'region:root';
  const rootRegion: GsplIrRegion = {
    id: rootId,
    name: 'root',
    nodes: [],
    subRegions: [],
...
function sortNodesCanonically(nodes: GsplIrNode[]): GsplIrNode[] {
  return [...nodes].sort((a, b) => a.id.localeCompare(b.id));
}

/** Deterministic edge ordering: sort by (from, to, kind) */
function sortEdgesCanonically(edges: GsplIrEdge[]): GsplIrEdge[] {
...
export function normalizeGraph(graph: GsplIrGraph): IrNormalizedGraph {
...
export function sortGraphCanonically(graph: GsplIrGraph): GsplIrGraph {
...
function makeHashable(graph: GsplIrGraph): string {
  const parts: string[] = [];
...
export function computeGraphHash(graph: GsplIrGraph): string {
...
export function validateGraphStructure(graph: GsplIrGraph): { ok: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  const nodesInRegions = new Set<NodeID>();
  for (const [, r] of graph.regions) {
    for (const nid of r.nodes) nodesInRegions.add(nid);
  }
