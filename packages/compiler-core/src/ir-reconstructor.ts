...
export interface ReconstructionContext { readonly geneRegistry: GeneTypeRegistry; readonly compilerVersion: string; readonly canonVersion: string; readonly limits: { readonly maxGenes: number; readonly maxConstraints: number; readonly maxDependencies: number; }; }
export interface IrReconstructionResult { ok: boolean; seed: CanonicalSeed; errors: string[]; warnings: string[]; }
...
export function reconstructSeedFromIr(graph, ctx) {
  ...
  var seedSchemaVersion = (schemaNode && schemaNode.value && schemaNode.value.schemaVersion) || "1.0";
    var identityNode = findMetaNode(graph.nodes, "identity");  var identityOverrides = {};  if (identityNode && identityNode.value) { var idv = identityNode.value; if (idv.authoredId !== undefined) identityOverrides.authoredId = idv.authoredId; if (idv.revisionId !== undefined) identityOverrides.revisionId = idv.revisionId; if (idv.lineageId !== undefined) identityOverrides.lineageId = idv.lineageId; if (idv.packageId !== undefined) identityOverrides.packageId = idv.packageId; }
var nsNode = findMetaNode(graph.nodes, "namespace");
  ...
  graph.nodes.forEach(function(node) {
    ...
    else { geneGroups.set(gn, { fragments: [frag], type: node.type, confidence: (node.attributes && node.attributes.confidence) || undefined }); }
  });
  var genes = {};
  ...
  var seed = makePrimordialSeed({
    ...
    if (identityOverrides.authoredId !== undefined) (seed).identity.authoredId = identityOverrides.authoredId;
  if (identityOverrides.revisionId !== undefined) (seed).identity.revisionId = identityOverrides.revisionId;
  if (identityOverrides.lineageId !== undefined) (seed).identity.lineageId = identityOverrides.lineageId;
  if (identityOverrides.packageId !== undefined) (seed).identity.packageId = identityOverrides.packageId;
  return { ok: errors.length === 0, seed: seed, errors: errors, warnings: [] };
}

export function verifyIndependentReconstruction( originalBytes: Uint8Array, graph, ctx) {
  ...
  if (match) { for (var i = 0; i < ob.length; i++) { if (ob[i] !== rb[i]) { match = false; break; } } }
  var fe = [];
  if (!match) {
    fe.push("Bytes diff: " + ob.length + " vs " + rb.length);
    if (JSON.stringify(originalSeed.payload) !== JSON.stringify(result.seed.payload)) fe.push("payload");
    if (JSON.stringify(originalSeed.constraints) !== JSON.stringify(result.seed.constraints)) fe.push("constraints");
    if (JSON.stringify(originalSeed.dependencies) !== JSON.stringify(result.seed.dependencies)) fe.push("dependencies");
  }
