...
export interface ConsistencyReport {
  ...
  determinism: { inventionsChecksum: number; sourcesChecksum: number; claimsChecksum: number };
}
...
export function checkLedgersVsJson(paths: {
  ...
  if (existsSync(paths.inventionsJson) && readFileSync(paths.inventionsJson, 'utf-8') !== invText) mismatches.push('inventions.json differs from in-memory regen');
  if (existsSync(paths.sourcesJson) && readFileSync(paths.sourcesJson, 'utf-8') !== srcText) mismatches.push('sources.json differs from in-memory regen');
  if (existsSync(paths.claimsJson) && readFileSync(paths.claimsJson, 'utf-8') !== claText) mismatches.push('claims.json differs from in-memory regen');

  if (existsSync(paths.inventionLedger)) {
    const md = readFileSync(paths.inventionLedger, 'utf-8');
    for (const inv of inventionsReg.inventions) if (!mentions(md, inv.id)) mismatches.push('invention ledger missing ' + inv.id);
  }
  if (existsSync(paths.provenanceRegistry)) {
    const md = readFileSync(paths.provenanceRegistry, 'utf-8');
    for (const s of sourcesReg.sources) if (!mentions(md, s.sourceId)) mismatches.push('provenance registry missing ' + s.sourceId);
  }
  if (existsSync(paths.claimsRegister)) {
    const md = readFileSync(paths.claimsRegister, 'utf-8');
    for (const c of claimsReg.claims) if (!mentions(md, c.claimId)) mismatches.push('claims register missing ' + c.claimId);
  }
