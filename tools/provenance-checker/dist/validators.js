 ...
 */
import { isArchitectureId, isInventionId, isValidClaimStatus, isValidDisposition, isValidTarget, } from './ids.js';
const ABSOLUTE_PATH_PREFIX = /^(?:[A-Za-z]:[\\/]|\/)/;
const ARCHIVE_URI_RE = /^archive:\/\/.+!\/.+/;
const VALID_IMPLEMENTATION_STATUS = new Set(['spec-only', 'partial', 'complete']);
export function checkProvenance(input) {
    ...
    // --- inventions ---
    const seenInventionIds = new Set();
    ...
    const fullIds = new Set(input.inventions.inventions.map((i) => i.id));
    for (const inv of input.inventions.inventions) {
        for (const d of inv.dependencies) {
            if (!fullIds.has(d))
                errors.push({ code: 'MISSING_DEPENDENCY', message: 'invention ' + inv.id + ' depends on unknown ' + d, ids: [inv.id] });
        }
        for (const c of inv.conflicts) {
            if (!fullIds.has(c))
                errors.push({ code: 'MISSING_CONFLICT', message: 'invention ' + inv.id + ' conflicts with unknown ' + c, ids: [inv.id] });
        }
    }
    // --- architecture decisions ---
    const seenDecisionIds = new Set();
    for (const d of input.decisions.decisions) {
        if (!isArchitectureId(d.id))
            errors.push({ code: 'BAD_DECISION_ID', message: 'decision id must be GSPL-ARCH-NNNN: ' + d.id, ids: [d.id] });
        if (seenDecisionIds.has(d.id))
            errors.push({ code: 'DUPLICATE_DECISION_ID', message: 'decision id duplicated: ' + d.id, ids: [d.id] });
        seenDecisionIds.add(d.id);
        if (!isValidDisposition(d.disposition))
            errors.push({ code: 'BAD_DECISION_DISPOSITION', message: 'decision disposition invalid: ' + d.disposition, ids: [d.id] });
        if (!d.rationale || d.rationale.length < 5)
            errors.push({ code: 'EMPTY_RATIONALE', message: 'decision rationale empty', ids: [d.id] });
        ...
        for (const r of d.references) {
            if (!fullIds.has(r))
                errors.push({ code: 'BAD_DECISION_REFERENCE', message: 'decision ' + d.id + ' references unknown invention ' + r, ids: [d.id] });
        }
    }
    // --- orphan detection ---
    const referencedByDecision = new Set();
    for (const d of input.decisions.decisions)
        for (const r of d.references)
            referencedByDecision.add(r);
    const citedByInvention = new Set();
    for (const inv of input.inventions.inventions) {
        for (const s of inv.evidence)
            citedByInvention.add(s.repo + ':' + s.file);
    }
    const orphaned = [];
    for (const id of fullIds) {
        if (!referencedByDecision.has(id))
            orphaned.push(id);
    }
    const orphanSources = [];
    for (const s of input.sources.sources) {
        const k = s.repositoryId + ':' + s.relativePath;
        if (!citedByInvention.has(k))
            orphanSources.push(s.sourceId);
    }
    // --- unresolved conflicts ---
    const dispById = new Map();
    for (const inv of input.inventions.inventions)
        dispById.set(inv.id, inv.disposition);
    const unresolved = [];
    const liveSet = new Set(['ADOPT', 'ADAPT', 'REWRITE']);
    for (const inv of input.inventions.inventions) {
        for (const cTarget of inv.conflicts) {
            if (!fullIds.has(cTarget))
                continue;
