...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        const a = argv[i];
        if (a === '--inventions' && i + 1 < argv.length)
            inv = argv[++i];
        else if (a === '--decisions' && i + 1 < argv.length)
            dec = argv[++i];
        else if (a === '--sources' && i + 1 < argv.length)
            src = argv[++i];
        else if (a === '--report' && i + 1 < argv.length)
            rpt = argv[++i];
        else if (a === '--known-repo-paths' && i + 1 < argv.length)
            known = argv[++i];
        else if (a === '--help' || a === '-h')
            return null;
    }
...
function help() {
    process.stdout.write(`gspl-provenance -- provenance validator for the GSPL canon\n\n` +
...
export async function main(argv) {
    ...
    const rawInv = JSON.parse(readFileSync(opts.inventionsPath, 'utf-8'));
    const rawDec = JSON.parse(readFileSync(opts.decisionsPath, 'utf-8'));
    const rawSrc = JSON.parse(readFileSync(opts.sourcesPath, 'utf-8'));
    ...
    process.stderr.write(`[gspl-provenance] report: ${opts.reportPath}\n` +
