 ...
 */
function isPermittedFixture(root, fullPath) {
...
export function checkCleanSource(repoRoot) {
    const issues = [];
            ...
            if (st.isDirectory()) {
                ...
                if (isForbiddenDir(entry)) {
                    if (!isPermittedFixture(root, full)) {
                        issues.push('Forbidden directory: ' + relative(root, full).replaceAll('\\', '/'));
                    }
                ...
                for (const pat of TEMP_DIR_PATTERNS) {
                    if (pat.test(entry)) {
                        if (!isPermittedFixture(root, full)) {
                            issues.push('Temporary/generated directory: ' + relative(root, full).replaceAll('\\', '/'));
                        }
                        flagged = true;
                        break;
                    }
                ...
                if (entry.endsWith('.tsbuildinfo')) {
                    if (!isPermittedFixture(root, full)) {
                        issues.push('Stale tsbuildinfo: ' + relative(root, full).replaceAll('\\', '/'));
                    }
                }
            }
        }
...
export async function main(argv) {
    ...
    if (result.ok)
        process.stderr.write('Clean source: PASS\n');
    else
        process.stderr.write('Clean source: FAIL (' + result.issues.length + ' issues)\n');
    return result.ok ? 0 : 2;
}
//# sourceMappingURL=check-clean-source.js.map