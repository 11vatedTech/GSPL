...
function canonicalizeValue(value, depth) {
    if (depth > 256)
        throw new Error('canonicalize: depth limit exceeded');
    if (value === null)
        return 'null';
    if (value === undefined)
        return 'null'; // RFC 8785 2.2.3: undefined in objects is dropped (we accept it at array position by treating it as null)
 ...
 */
function stripIdentityFields(seed) {
    const out = {};
    ...
    for (const k of Object.keys(raw)) {
        if (HASH_EXCLUSION_KEYS.has(k))
            continue;
        const v = raw[k];
        if (k === '$lineage' && isPlainObject(v)) {
            const lin = {};
            for (const lk of Object.keys(v)) {
                if (!TIMESTAMP_EXCLUSION_KEYS.has(lk))
                    lin[lk] = v[lk];
            }
            out[k] = lin;
        }
 ...
 */
export function canonicalize(seed) {
 ...
 */
export function canonicalizeGene(gene) {
 ...
 */
export function canonicalizeAny(value) {
