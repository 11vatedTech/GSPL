...
export function checkDeterminism(repoRoot: string): { ok: boolean; artifacts: string[] } {
  const run = (cmd: string) => { try { execSync(cmd, { cwd: repoRoot, stdio: 'pipe' }); } catch {} };
  const hash = (f: string, exclude: string[]) => {
    const obj = JSON.parse(readFileSync(f, 'utf-8'));
    exclude.forEach((k) => delete (obj as any)[k]);
  ...
  const ok = keys.every((k) => (h1 as any)[k] === (h2 as any)[k]);
