...
export const SEVEN_AXES_COUNT = SEVEN_AXES.length;
function evidenceFor(seed, axis) {
    const missing = [];
    const present = [];
    switch (axis) {
        case 'signed':
        ...
        case 'typed':
                ...
                for (const [name, g] of Object.entries(seed.genes)) {
                    if (g && g.type) {
                        present.push({
                            ...
                            present: true,
                            note: `gene '${name}' has type '${String(g.type)}'`,
                        });
                    }
                    else {
            ...
            if (!seed.$domain)
                missing.push({ path: '$.$domain', present: false, note: 'no $domain' });
            else
                present.push({ path: '$.$domain', present: true, note: 'domain declared' });
            break;
        case 'lineage-tracked': {
            ...
            const hasParents = !!(seed.$lineage && Array.isArray(seed.$lineage.parents) && seed.$lineage.parents.length > 0);
        ...
        case 'confidence-bearing':
            ...
            for (const [, g] of Object.entries(seed.genes)) {
                total += 1;
                if (g && typeof g.confidence === 'number')
                    withConf += 1;
            }
 ...
 */
export function checkSevenAxes(seed) {
    const axes = SEVEN_AXES.map((axis) => {
