...
export interface ClassifyOptions {
  ...
  expectedSteps?: readonly { from: ClaimStatus; to: ClaimStatus; since: string }[];
}

export function classify(registry: ClaimRegistry, options: ClassifyOptions = {}): ClaimCheckReport {
  const errors: ClaimError[] = [];
  const warnings: ClaimError[] = [];
  ...
  const illegalTransitions: { claimId: string; from: ClaimStatus; to: ClaimStatus; since: string }[] = [];
