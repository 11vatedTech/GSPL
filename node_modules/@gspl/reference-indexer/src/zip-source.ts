...
function buildRecord(
  repo: RepoConfig, relPath: string,
  classification: FileClassification,
  hashed: { sha256: string; bytes: number; lines: number },
): ReferenceFileRecord {
...
export class DirectorySource implements ReferenceSource {
  async scan(repo: RepoConfig): Promise<readonly ReferenceFileRecord[]> {
    const sourcePath = repo.source;
    const stats = await stat(sourcePath).catch(() => null);
    if (!stats || !stats.isDirectory()) return [];
    const records: ReferenceFileRecord[] = [];
    let excluded = 0, unreadable = 0;
    const walk = async (absDir: string): Promise<void> => {
      let entries: string[];
      try { entries = await readdir(absDir); entries.sort(); } catch { return; }
      for (const entry of entries) {
        const absChild = join(absDir, entry);
        const childStat = await stat(absChild).catch(() => { unreadable++; return null; });
        if (!childStat) continue;
        if (childStat.isDirectory()) {
          if (shouldExcludeDir(entry)) { excluded++; continue; }
          await walk(absChild);
        } else if (childStat.isFile()) {
          if (shouldExcludeFile(entry)) { excluded++; continue; }
          const relPath = normalizePath(relative(sourcePath, absChild));
          const classification = classifyFile(relPath);
          if (classification === 'binary' && childStat.size > 10 * 1024 * 1024) { excluded++; continue; }
...
export class ZipArchiveSource implements ReferenceSource {
  async scan(repo: RepoConfig): Promise<readonly ReferenceFileRecord[]> {
    ...
    const records: ReferenceFileRecord[] = [];
    return new Promise<readonly ReferenceFileRecord[]>((resolve, reject) => {
      yauzl.open(zipPath, { lazyEntries: true }, (err, zipfile) => {
        if (err) { reject(err); return; }
        if (!zipfile) { resolve([]); return; }
        ...
        zipfile.on('entry', (entry: any) => {
          ...
          if (normalized.startsWith('/')) {
            rejected++; zipfile.readEntry(); return;
          }
          const segments = normalized.split('/');
          if (segments.some((s: string) => ZIP_SLIP_SEGMENTS.has(s))) {
            rejected++; zipfile.readEntry(); return;
          }
          if (entry.fileName.endsWith('/') || entry.fileName.endsWith(String.fromCharCode(92))) { zipfile.readEntry(); return; }
          const dirParts = segments.slice(0, -1);
          const base = segments[segments.length - 1];
          if (dirParts.some((d: string) => shouldExcludeDir(d))) { excluded++; zipfile.readEntry(); return; }
          if (shouldExcludeFile(base)) { excluded++; zipfile.readEntry(); return; }
          const classification = classifyFile(normalized);
          zipfile.openReadStream(entry, (streamErr, stream) => {
            if (streamErr) { corrupted++; zipfile.readEntry(); return; }
            if (!stream) { zipfile.readEntry(); return; }
            const chunks: Buffer[] = [];
            stream.on('data', (chunk: Buffer) => chunks.push(chunk));
            stream.on('end', () => {
              const raw = Buffer.concat(chunks);
              const h = createHash('sha256');
              let bytes: number; let lines = 0;
              if (classification === 'binary') { h.update(raw); bytes = raw.length; }
              else {
